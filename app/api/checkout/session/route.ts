...
import { fetchVcoProducts, getVcoConfig, publicCheckoutError, resolveProgramProduct, sanitizeCouponCode } from '../../../../lib/vco'
...
export async function POST(req: NextRequest) {
  ...
  if (!isValidEmail(email) || !isValidPhone(phone) || !isValidAdultDob(dob) || !/^[A-Z]{2}$/.test(state)) {
    return NextResponse.json(
      { error: 'Some intake details could not be verified. Please review your information and try again.' },
      { status: 400 },
    )
  }
  ...
  try {
    const res = await fetch(`${baseUrl}/api/commerce/v1/checkout/session`, {
      ...
      body: JSON.stringify({
        ...
        cancelUrl: `${origin}/get-started?canceled=1`,
      }),
    })
    ...
    if (!res.ok || !data.checkoutUrl) {
      ...
      return NextResponse.json(
        { error: publicCheckoutError(res.status, data.error) },
        { status: res.status >= 400 ? res.status : 502 },
      )
    }

    return NextResponse.json({ checkoutUrl: data.checkoutUrl, orderId: data.orderId })
  } catch {
